df<-fread("GSE235863_bulk_rna_seq_raw_counts.txt")
library(data.table)
library(tidyverse)
library(data.table)
library(DESeq2)
...
count2_filt$geneName<-NULL

colnames(count2_filt)
## 3. 读取样本信息
metadata <- read.csv("GSE235863_HCC_clinical_metadata.csv")
...
metadata$response <- factor(metadata$response, levels = c("Responder", "Non_responder"))
...
res <- results(dds, contrast = c("response", "Responder", "Non_responder"))
